...
At the end of program execution, the result is shown in a formatted console output to check the alignment {\color{blue}{(Sup.Fig. \ref{fig:output})}}. The result is also written in the output file at the path provided at program execution.
...
The first notable remark is that the program execution time seem to be proportional to the square of traces lengths {\color{blue}{(Sup.Fig. \ref{sub:graph1}})}.
...
The quantity of top against top and event against event matches seem to be linearly proportional to the traces length {\color{blue}{(Sup.Fig. \ref{sub:graph3}})}.
...
After this threshold, execution time starts to rapidly increase going to $\approx$250 milliseconds for traces over 100 elements {\color{blue}{(Sup.Fig. \ref{sub:graph1})}}. 
